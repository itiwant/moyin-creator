...
export function ShotListPanel({ onGenerateImage }: ShotListPanelProps) {
  ...
  const handleBatchGenerate = async () => {
    ...
    if (allStartFramesGenerated && !confirm("Bạn có chắc muốn tạo lại tất cả khung hình đầu không?")) {
      return;
    }
    ...
    for (let i = 0; i < shotsToProcess.length; i++) {
      ...
      setBatchProgress((prev) => ({
        ...prev,
        current: i + 1,
        message: `TạoCảnh quay ${i + 1}/${shotsToProcess.length}`,
      }));
  ...
  const getSceneName = (sceneRefId: string) => {
    ...
    return scene?.name || scene?.location || `Cảnh ${sceneRefId}`;
  };
    ...
    return (
      <div className="h-full flex flex-col">
        <div className="p-3 border-b border-border">
          <h3 className="font-medium text-sm">Danh sách cảnh quay</h3>
        </div>
        <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground p-4">
          <AlertCircle className="w-10 h-10 mb-3 opacity-30" />
          <p className="text-sm text-center">Chưa có cảnh quay</p>
          <p className="text-xs text-center mt-1 opacity-60">
            Vui lòng tạo phân cảnh ở giai đoạn kịch bản trước
          </p>
        </div>
      </div>
    );
      ...
      <div className="p-3 border-b border-border space-y-2">
        ...
        <div className="flex items-center justify-between">
          ...
          >
            ...
            {allStartFramesGenerated ? "Tạo lại" : "Tạo hàng loạt"}
          </Button>
        </div>
      </div>
                  ...
                  >
                    ...
                    <div className="p-1.5 bg-background">
                      <p className="text-[10px] text-muted-foreground line-clamp-1">
                        {shot.shotSize || shot.cameraMovement || "Cảnh quay"}
                      </p>
                    </div>
                  </div>
                );
