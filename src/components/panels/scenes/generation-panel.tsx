...
export function GenerationPanel({ selectedScene, onSceneCreated }: GenerationPanelProps) {
  ...
  useEffect(() => {
    ...
    if (currentPage && contactSheetPrompt) {
      ...
      // 2.5 从gốc英文promptđang xử lý...Scene Context 和 Visual Description
...
Phong cách：${stylePreset?.name || 'Phong cách hoạt hình'}，Tất cảôánh sáng一致，ô之间用细白边框ngăn cách，chỉ có nền，không có nhân vật。`;
  ...
  /**
   * Trực tiếpTạo联合图（gọi API内部 AI Tạo ảnh API）
  ...
  const handleSaveViewpointImages = async () => {
    ...
    for (const vp of viewpointsToUse) {
      ...
      // 验证本地Lưu是否Thành công（Thất bại时 saveImageToLocal Quay lạigốc data: URL）
      if (!localPath.startsWith('local-image://')) {
        console.warn(`[ContactSheet] Góc nhìnảnh本地LưuThất bại: ${vp.name}, 将使用gốc URL`);
      }
  ...
  const handleAutoGenerateContactSheet = async () => {
    ...
    (async () => {
      try {
        ...
        setContactSheetTask(parentSceneId, { status: 'generating', progress: 30, message: '正在gọi API AI Tạo...' });
...
No characters, empty environment.`;
        ...
        // 收 tậpẢnh tham chiếu：优先用「Toàn cảnh」conCảnh + 当前conCảnhảnh
  ...
  const extractSpatialAssets = (scene: Scene) => {
    ...
    const commonAnchors = ['桌', '椅', '床', '沙发', '柜', '台', '架', '灯', '门', ''];
...
使用平坦、đang xử lý...影ánh sáng以确保纹理可见。无Nhân vật，空Cảnh。`;
  ...
  const handleGenerateOrthographicImage = async () => {
    ...
    try {
      ...
      // 收 tậpẢnh tham chiếu：优先使用「Toàn cảnh」conCảnh，而不是整张联合图
      ...
      <div className="h-full flex flex-col">
        ...
        <ScrollArea className="flex-1 p-3">
          <div className="space-y-4">
            ...
            {/* prompt（Mặc địnhMở rộng，可Chỉnh sửa，根据Ngôn ngữ偏好只Hiện一种） */}
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="详细Mô tảCảnh的môi trường，例如：繁华的东京涩谷十字路sổ，霓虹灯闪烁..."
