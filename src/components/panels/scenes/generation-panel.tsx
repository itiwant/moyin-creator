...
export function GenerationPanel({ selectedScene, onSceneCreated }: GenerationPanelProps) {
  ...
  // 获取当前项目的Phân cảnh数据，用于提取CảnhĐạo cụ
  ...
  const [visualPrompt, setVisualPrompt] = useState(""); // Cảnh视觉Mô tả
  ...
  // Generation mode: single (单图), contact-sheet (联合图/多Góc nhìn), orthographic (四góc nhìn)
  ...
  // 批量四góc nhìnTrạng thái
  const [savedChildSceneIds, setSavedChildSceneIds] = useState<string[]>([]); // 刚Lưu的conCảnh ID
  ...
  // 当从Kịch bản跳转过来时，Tự độngTạoCảnh并进入联合图Tạochế độ
  useEffect(() => {
    if (!pendingSceneData) return;
    
    // 立即捕获数据并清除，防止 React 严格chế độ下重复执 hàng
    ...
    if (data.name && data.location) {
      ...
      // 如果有多Góc nhìn数据，Trực tiếp进入联合图Tạochế độ
      if (data.viewpoints && data.viewpoints.length > 0 &&
        ...
        // 同步布局Cài đặt，确保切割时使用正确的 hàng cột数
        if (firstPage.gridLayout) {
          ...
          // 根据Tỷ lệ khung hìnhCài đặt方向：正Vuông网格（3x3, 2x2）Mặc địnhNgang
          if (cols >= rows) {
  ...
  // 当用户thay đổiTỷ lệ khung hình时，根据Góc nhìn数量重新计算最优布局
  // 注意：不重新提取Góc nhìn，只更新布局和prompt
  useEffect(() => {
    ...
    // 根据Góc nhìn数量和Tỷ lệ khung hình计算最优布局
    // 强制使用 N x N 布局以保证Tỷ lệ khung hình一致性
    let newLayout: { rows: number; cols: number };
    
    // 如果Góc nhìn数量 <= 4，使用 2x2
    // 如果Góc nhìn数量 > 4，使用 3x3
    if (vpCount <= 4) {
    ...
    // Tạo lại当前页的prompt（替换 hàng cột数）
    ...
    if (currentPage && contactSheetPrompt) {
      ...
      // 2.5 从原始英文promptđang xử lý...Scene Context 和 Visual Description
      ...
      // 从原始prompt tiếng Trungđang xử lý...ô tả cảnh（Phong cách kiến trúc、Bảng màu sắc、Đặc trưng thời đại、Thiết kế ánh sáng）
      // 这样即使 selectedScene 还没更新，也能保留正确的Mô tả cảnh
      let sceneDescZh = '';
      let visualPromptZh = '';
      const originalPromptZh = currentPage.promptZh || '';
      
      // Mô tả cảnh在第一 hàng和"CảnhBầu không khí"或"X ô分别Hiển thị"之间
      ...
      const newPromptZh = `一张精确的 ${newLayout.rows} hàng${newLayout.cols} cột ảnh lưới（共 ${totalCells} ô），Hiển thị同一「${sceneName}」Cảnh的Các góc nhìn khác nhau.
${sceneDescZh}${visualPromptZh ? `\nCảnhBầu không khí：${visualPromptZh}` : ''}

${totalCells} ô分别Hiển thị：
${gridItemsZh}

重要：
- 必须精确Tạo ${newLayout.rows}  hàng ${newLayout.cols}  cột，không được nhiều hơn cũng không ít hơn。
- Đây là ảnh tham chiếu sạch,Ảnh tham chiếu， không thêmThêmbất kỳ văn bản nào。
- KhôngThêmThẻ、tiêu đề、văn bản mô tả、hình mờ或任何Loại的文字。

Phong cách：${stylePreset?.name || 'Phong cách hoạt hình'}，Tất cảôánh sáng一致，ô之间用细白边框ngăn cách，chỉ có nền，không có nhân vật。`;
  ...
  }, [contactSheetAspectRatio]); // 只监听Tỷ lệ khung hình变化
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const actionDescriptions = sceneShots
        ...
        .filter(Boolean)
        .slice(0, 10); // 最多取 10 Phân cảnh
      
      console.log('[SceneGeneration] 找到', sceneShots.length, 'Phân cảnh用于Cảnh:', selectedScene?.name);
      console.log('[SceneGeneration] Hành động描写:', actionDescriptions);
      ...
      toast.success("Cảnh概念图Tạo完成，请Xem trướcXác nhận");
    } catch (error) {
  ...
  const handleSavePreview = async () => {
    ...
    try {
      ...
      toast.success("Cảnh概念图đã lưu到本地！", { id: 'saving-scene-preview' });
    } catch (error) {
  ...
  /**
   * Tạo多Góc nhìn联合图prompt
   */
  const handleGenerateContactSheetPrompt = () => {
    if (!selectedScene) {
      toast.error("请先ChọnCảnh");
      return;
    }
    ...
    if (sceneShots.length === 0) {
      toast.warning("该Cảnh没有关联的Phân cảnh，将使用Mặc địnhGóc nhìn");
    }
    ...
    // 检查是否使用了 AI 分析的Góc nhìn
    // viewpoints 属性可能来自Kịch bản的 scriptData.scenes，通过 pendingSceneData 传递
  ...
  const handleCopyPrompt = (isEnglish: boolean) => {
    ...
    if (isEnglish) {
      ...
      fullPrompt = [
        ...
        `Phong cáchquan trọng词: ${styleTokens}`,
    ...
    toast.success(isEnglish ? "英文prompt已Sao chép（含Phong cách和Tỷ lệ khung hình）" : "prompt tiếng Trung已Sao chép（含Phong cách和Tỷ lệ khung hình）");
  };

  /**
   * Trực tiếpTạo联合图（调用内部 AI Tạo ảnh API）
   * 使用 submitGridImageRequest 对齐Đạo diễn面板，确保网格格式正确
   */
  const handleGenerateContactSheetImage = async () => {
    ...
    try {
      ...
      // 增强prompt：如果用户Chỉnh sửa的是prompt tiếng Trung，在前面包裹英文Cấu trúc化网格指令
      ...
      if (!result.imageUrl) {
        throw new Error('Tạo ảnh thất bại：未Quay lạiảnh URL');
      }
      
      // 如果Quay lại的是 HTTP URL，转为 base64 — 避免后续切割时 CORS 问题
      ...
      if (finalImageUrl.startsWith('http://') || finalImageUrl.startsWith('https://')) {
        try {
          ...
          console.warn('[ContactSheet] HTTP→base64 转换Thất bại，使用原URL');
        }
      }
      
      setContactSheetImage(finalImageUrl);
      toast.success("联合图Tạo thành công，可以thực hiện切割");
    } catch (error) {
      const err = error as Error;
  ...
  /**
   * Tải lên联合图（备用，用于Thủ côngTải lên外部Tạo的ảnh）
   */
  const handleUploadContactSheet = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      toast.success("联合图đã tải lên，可以thực hiện切割");
    };
    reader.readAsDataURL(file);
  };

  /**
   * 独立Tải lên联合图入口（不需要先Tạoprompt）
   * 根据用户Chọn的网格布局Tự độngTạoMặc địnhGóc nhìn
   * 重要：HủyĐang chọn的Cảnh，确保Lưu时Tạo新Cảnh
   */
  const handleDirectUploadContactSheet = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;

    // 重要：HủyĐang chọn的Cảnh，确保Lưu时会Tạo新Cảnh
    selectScene(null);
    
    // 清空表单，准备Tự độngđặt tên
    ...
    // TạoMặc định的prompt页面数据（用于切割时获取布局信息）
    ...
    // Cài đặt一Placeholderprompt，触发进入联合图界面
    setContactSheetPrompt('[Trực tiếpTải lên - 无prompt]');
    setContactSheetPromptZh('[Trực tiếpTải lên - 无prompt]');
    ...
    reader.onload = (event) => {
      const dataUrl = event.target?.result as string;
      setContactSheetImage(dataUrl);
      toast.success(`联合图đã tải lên（${dims.rows}×${dims.cols} = ${totalCells}格），切割后将Tự độngTạo新Cảnh`);
    };
    reader.readAsDataURL(file);
  };

  /**
   * 在联合图界面đang xử lý...局变化（仅对Trực tiếpTải lênchế độ生效）
   * 更新Góc nhìn数量以匹配新布局
   */
  const handleContactSheetLayoutChange = (newLayout: ContactSheetLayout) => {
    setContactSheetLayout(newLayout);
    
    // 如果是Trực tiếpTải lênchế độ（没有真正的prompt），需要更新Góc nhìn数据
    if (contactSheetPrompt === '[Trực tiếpTải lên - 无prompt]') {
  ...
  const handleSplitContactSheet = async () => {
    // 优先使用 pendingViewpoints（从Kịch bản传来的），否则用 extractedViewpoints
    const currentPageVps = pendingViewpoints.filter(v => v.pageIndex === currentPageIndex);
    ...
    if (!contactSheetImage || viewpointsToUse.length === 0) {
      toast.error("请先Tải lên联合图并Tạoprompt");
      return;
    }

    setIsSplitting(true);
    try {
      // 优先从 pendingContactSheetPrompts 获取布局（这是Tạoprompt时Xác nhận的真实布局）
      // 如果没有，才使用用户Chọn的 contactSheetLayout
      ...
      if (currentPagePrompt?.gridLayout) {
        // 使用Tạoprompt时Xác nhận的布局
        expectedRows = currentPagePrompt.gridLayout.rows;
        expectedCols = currentPagePrompt.gridLayout.cols;
        console.log('[Split] 使用 pendingContactSheetPrompts đang xử lý...:', { expectedRows, expectedCols });
      } else {
        // 后备：使用用户Chọn的布局
        const dims = getLayoutDimensions(contactSheetLayout, contactSheetAspectRatio);
        expectedRows = dims.rows;
        expectedCols = dims.cols;
        console.log('[Split] 使用用户Chọn的布局:', { expectedRows, expectedCols, contactSheetLayout });
      }
      
      const expectedCount = expectedRows * expectedCols;
      
      // 如果ảnh是 HTTP URL，先转为 base64 避免 CORS 导致 canvas 被污染
      ...
      if (contactSheetImage.startsWith('http://') || contactSheetImage.startsWith('https://')) {
        ...
        try {
          ...
          console.warn('[Split] HTTP→base64 转换Thất bại，使用原URL:', convertErr);
        }
      }
      ...
      // 同步更新 extractedViewpoints，确保Lưu时有数据
      ...
      toast.success(`已切割为 ${Object.keys(viewpointImagesMap).length} Góc nhìnảnh`);
    } catch (error) {
      console.error('[ContactSheet] 切割Thất bại:', error);
      toast.error("切割Thất bại，请检查ảnh格式");
    } finally {
  ...
  /**
   * LưuGóc nhìnảnh - 为每Góc nhìnTạo独立的conCảnh
   * 例如：“张家客厅” → TạoThư mục“张家客厅-Góc nhìn” → LưuconCảnh到Thư mục
   * 如果没有đã chọnCảnh，会Tự độngTạo一父Cảnh
   */
  const handleSaveViewpointImages = async () => {
    if (Object.keys(splitViewpointImages).length === 0) {
      toast.error("没有可Lưu的Góc nhìnảnh");
      return;
    }
    
    // 如果没有đã chọnCảnh，先Tự độngTạo一父Cảnh
    ...
    if (!parentScene) {
      ...
      if (!parentScene) {
        toast.error("Tạo父CảnhThất bại");
        return;
      }
      
      // đã chọn新Tạo的Cảnh
      selectScene(newParentId);
      toast.success(`已Tự độngTạoCảnh「${sceneName}」`);
    }
    ...
    if (viewpointsToUse.length === 0) {
      toast.error("没有Góc nhìn数据");
      return;
    }
    
    // === 补全未分配的Phân cảnh shotIds ===
    // 找到当前Cảnh的Tất cảPhân cảnh
    const sceneName = parentScene.name || parentScene.location || '';
    const sceneShots = allShots.filter(shot => {
      // 通过 sceneRefId 或Tên cảnh匹配
    ...
    if (sceneShots.length > 0) {
      ...
      if (unassignedShots.length > 0) {
        console.log(`[ContactSheet] 发现 ${unassignedShots.length} 未分配的Phân cảnh，按序号分配到Góc nhìn`);
        
        // 按Phân cảnh序号分配到Góc nhìn（Phân cảnh1->Góc nhìn1，Phân cảnh2->Góc nhìn2，...）
        ...
        for (const shot of unassignedShots) {
          // 根据Phân cảnh在Cảnh内的序号Xác nhận对应的Góc nhìn
    ...
    for (const vp of viewpointsToUse) {
      ...
      // 验证本地Lưu是否Thành công（Thất bại时 saveImageToLocal Quay lại原始 data: URL）
      if (!localPath.startsWith('local-image://')) {
        console.warn(`[ContactSheet] Góc nhìnảnh本地LưuThất bại: ${vp.name}, 将使用原始 URL`);
      }
    ...
    console.log('[ContactSheet] Lưu完成（始终Tạo mới）:', {
      parentSceneId: parentScene.id,
      created: createdVariantIds.length,
    });

    // 仅Lưu新Tạo的conCảnh ID，用于批量四góc nhìn
   ...
   * 任务在后台运 hàng，用户可以Tiếp tụcCài đặt下一Tạo任务
   */
  const handleAutoGenerateContactSheet = async () => {
    ...
    console.log('[AutoContactSheet] 快照Trạng thái:', {
    ...
    setContactSheetTask(parentSceneId, { status: 'generating', progress: 10, message: 'Đang tạo联合图...' });
    toast.info(`Cảnh「${snapshotName}」联合图Bắt đầuTạo...`);

    // 立即清空左栏Trạng thái，允许用户Cài đặt下一任务
    setContactSheetPrompt(null);
    setContactSheetPromptZh(null);
    setContactSheetImage(null);
    setSplitViewpointImages({});
    ...
    (async () => {
      try {
        // ==================== 阶段 1: Tạo联合图 ====================
        // 获取 API 配置 — 与Đạo diễn面板一致使用 submitGridImageRequest
        ...
        const isChinese = /[\u4e00-\u9fa5]/.test(finalPrompt) && !finalPrompt.includes('<instruction>');
        if (isChinese) {
          // 用户提供了prompt tiếng Trung但没有Cấu trúc化指令 → 包裹英文 grid 指令
          ...
          // 已有英文Cấu trúc化prompt，追加Prompt phủ định
          if (!finalPrompt.includes('Negative constraints:')) {
            finalPrompt += `\nNegative constraints: ${negativePrompt}`;
          }
        }

        setContactSheetTask(parentSceneId, { status: 'generating', progress: 30, message: '正在调用 AI Tạo...' });

        // 使用 submitGridImageRequest — 与Đạo diễn面板保持一致
        const result = await submitGridImageRequest({
          model,
          prompt: finalPrompt,
          apiKey,
        ...
        console.log('[AutoContactSheet] 阶段1完成，ảnhURLLoại:', 
        ...
        if (generatedImageUrl.startsWith('http://') || generatedImageUrl.startsWith('https://')) {
          ...
          try {
            ...
            console.log('[AutoContactSheet] HTTP→base64 转换Thành công，长度:', imageForSplit.length);
          } catch (convertErr) {
            console.warn('[AutoContactSheet] HTTP→base64 转换Thất bại，使用原URL:', convertErr);
          }
        }
        ...
        // 如果 snapshotViewpoints 为空（用户Thủ côngChỉnh sửaprompt，未走Góc nhìnTạo流程），
        // Tự độngTạo fallback Góc nhìn以匹配切割结果
        let effectiveViewpoints = snapshotViewpoints;
        if (effectiveViewpoints.length === 0 && splitResults.length > 0) {
          console.log('[AutoContactSheet] Góc nhìn为空，Tự độngTạo fallback Góc nhìn，数量:', splitResults.length);
        ...
        for (const vp of effectiveViewpoints) {
          ...
          if (splitResult) {
            ...
            if (splitResult.row !== expectRow || splitResult.col !== expectCol) {
              console.warn(`[AutoContactSheet] Trực tiếp索引不匹配: gridIndex=${gridIdx}, split[row=${splitResult.row},col=${splitResult.col}] vs expected[row=${expectRow},col=${expectCol}]`);
          ...
          if (splitResult) {
            ...
            console.warn(`[AutoContactSheet] Góc nhìn ${vp.name}(gridIndex=${gridIdx}) 未找到对应切割结果`);
          }
        }
        ...
        // ===== 安全回退：如果映射Tất cảThất bại但切割有结果，Trực tiếp使用切割结果TạoconCảnh =====
        if (mappedCount === 0 && splitResults.length > 0) {
          console.warn('[AutoContactSheet] ⚠ 映射Tất cảThất bại！Bật安全回退：Trực tiếp使用切割结果TạoconCảnh');
        ...
        if (!parentScene) {
          throw new Error('父Cảnh已被Xóa');
        }
        const parentSceneName = parentScene.name || parentScene.location;
        const targetFolderId = parentScene.folderId;
        const createdVariantIds: string[] = [];

        // 补全Phân cảnh shotIds — 使用 effectiveViewpoints（含 fallback）
        ...
        console.log('[AutoContactSheet] 阶段3: 准备LưuconCảnh, viewpointsToSave:', viewpointsToSave.length, 'viewpointImagesMap条目:', Object.keys(viewpointImagesMap).length);

        for (const vp of viewpointsToSave) {
          const imgData = viewpointImagesMap[vp.id];
          if (!imgData) {
            console.warn(`[AutoContactSheet] 跳过Góc nhìn ${vp.name}: viewpointImagesMap đang xử lý...数据 (id=${vp.id})`);
            continue;
          }
        ...
        // Lưu联合图到父Cảnh（同时tương thích base64 和 imageForSplit 已转换过的）
        ...
        setContactSheetTask(parentSceneId, { status: 'done', progress: 100, message: `完成，đã tạo ${createdVariantIds.length} conCảnh` });
        if (createdVariantIds.length > 0) {
          toast.success(`Cảnh「${parentSceneName}」联合图已切割Lưu，共 ${createdVariantIds.length} Góc nhìnconCảnh（NhấpMở rộng查看）`);
        } else {
          toast.warning(`Cảnh「${parentSceneName}」联合图đã lưu，但未能TạoconCảnh（切割结果: ${splitResults.length} ）`);
        }
        ...
        const err = error as Error;
        console.error('[AutoContactSheet] Tự động流水线Thất bại:', err);
        setContactSheetTask(parentSceneId, { status: 'error', progress: 0, message: err.message });
        toast.error(`Cảnh联合图Tự độngTạo thất bại: ${err.message}`);
        // 10秒后清除错误Trạng thái
        setTimeout(() => {
          setContactSheetTask(parentSceneId, null);
        }, 10000);
  ...
  const handleBatchGenerateOrthographic = async () => {
    ...
    if (childScenes.length === 0) {
      toast.error("找不到conCảnh");
      return;
    }

    toast.info(`Bắt đầu为 ${childScenes.length} conCTạo ảnh四góc nhìn...`);
...
No characters, empty environment.`;
        ...
        // 收集Ảnh tham chiếu：优先用「Toàn cảnh」conCảnh + 当前conCảnhảnh
        const rawReferenceImages: string[] = [];
        
        // 1. 获取同一父Cảnh下的「Toàn cảnh」conCảnh
        ...
        if (overviewImage) {
          ...
          console.log(`[批量四góc nhìn] ${childScene.name} 使用Toàn cảnhconCảnh作为Tham chiếu`);
        }
        ...
        successCount++;
        console.log(`[批量四góc nhìn] ${childScene.name} 完成`);
      } catch (err) {
        failCount++;
        console.error(`[批量四góc nhìn] ${childScene.name} Thất bại:`, err);
      }
    }

    setSavedChildSceneIds([]);
    toast.success(`批量四góc nhìn完成！Thành công ${successCount} ，Thất bại ${failCount} `);
  };

  // ========== 四góc nhìn（正交góc nhìn）功能 ==========
  ...
  /**
   * Tạo bốn góc nhìn（正交góc nhìn）prompt
   */
...
Unified by flat, neutral cinematic lighting to ensure texture visibility. No characters, empty environment.`;
    ...
    // prompt tiếng Trung
    const promptZh = `专业正交概念图，精确的 2x2 网格排 cột，Hiển thị「${sceneName}」的四基本Góc nhìn，保持完美的空间连续性。${stylePreset?.name || 'Phong cách hoạt hình'}，详细的môi trường概念艺术。

**左上（chính diệngóc nhìn）：**
${anchor} 的chính diện直视Ống kính。清晰Hiển thịchính diện细节。背景是其后方的墙壁，包含 ${walls.south}。

**右上（mặt saugóc nhìn）：**
${anchor} 的mặt sau直视Ống kính。Hiển thị后部Cấu trúc。背景是物体面向的墙壁，包含 ${walls.north}。

**左下（左侧góc nhìn）：**
从左侧拍摄的 ${anchor} bên cạnhỐng kính。背景是对面的墙壁，严格包含 ${walls.east}。

**右下（右侧góc nhìn）：**
从右侧拍摄的 ${anchor} bên cạnhỐng kính。背景是对面的墙壁，严格包含 ${walls.west}。

使用平坦、đang xử lý...影ánh sáng以确保纹理可见。无Nhân vật，空Cảnh。`;

    setOrthographicPrompt(promptEn);
    setOrthographicPromptZh(promptZh);
    toast.success("四góc nhìnpromptđã tạo");
  };
  ...
  const handleGenerateOrthographicImage = async () => {
    ...
    try {
      ...
      // 收集Ảnh tham chiếu：优先使用「Toàn cảnh」conCảnh，而不是整张联合图
      ...
      if (selectedScene?.parentSceneId) {
        ...
        if (overviewScene?.referenceImage) {
          overviewImage = overviewScene.referenceImage;
          console.log('[Orthographic] 找到Toàn cảnhconCảnh作为Tham chiếu');
        }
        ...
        if (!overviewImage) {
          ...
          if (overviewByName?.referenceImage) {
            overviewImage = overviewByName.referenceImage;
            console.log('[Orthographic] 按Tên找到Toàn cảnhconCảnh');
          }
        }
      }
      ...
      if (overviewImage) {
        ...
        console.log('[Orthographic] 使用Toàn cảnhconCảnh作为主Tham chiếu');
      }
      ...
      if (selectedScene?.referenceImage && selectedScene.referenceImage !== overviewImage) {
        ...
        console.log('[Orthographic] ThêmconCảnhảnh作为辅助Tham chiếu');
      }
      ...
      toast.success("四góc nhìnTạo thành công，可以thực hiện切割");
    } catch (error) {
      const err = error as Error;
  ...
  const handleUploadOrthographic = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      toast.success("四góc nhìnđã tải lên，可以thực hiện切割");
    };
    reader.readAsDataURL(file);
  };
  ...
  const handleSplitOrthographic = async () => {
    if (!orthographicImage) {
      toast.error("Vui lòng tạo trước或Tải lên四góc nhìn");
      return;
    }

    setIsSplitting(true);
    try {
      // 2x2 切割，Hỗ trợ 16:9 或 9:16
      const splitResults = await splitStoryboardImage(orthographicImage, {
        aspectRatio: orthographicAspectRatio, // 使用用户Chọn的Tỷ lệ khung hình
        resolution: '2K',
        sceneCount: 4,
        options: {
          expectedRows: 2,
      ...
      // 映射到四Góc nhìn: 左上=chính diện, 右上=mặt sau, 左下=左侧, 右下=右侧
      ...
      toast.success("已切割为 4 Góc nhìnảnh");
    } catch (error) {
  ...
  const handleSaveOrthographicViews = async () => {
    ...
    // Lưu四góc nhìnẢnh gốc到父Cảnh
    updateScene(selectedScene.id, {
      orthographicImage,
    } as any);

    toast.success(`đã tạo ${createdIds.length} 正交Góc nhìnCảnh`);
  ...
  const handleCopyOrthographicPrompt = (isEnglish: boolean) => {
    ...
    navigator.clipboard.writeText(fullPrompt);
    toast.success(isEnglish ? "英文prompt已Sao chép" : "prompt tiếng Trung已Sao chép");
  };
      ...
      <div className="h-full flex flex-col">
        <div className="p-3 pb-2 border-b flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-medium text-sm">四góc nhìn（正交góc nhìn）</h3>
          </div>
        ...
        <ScrollArea className="flex-1 p-3">
          <div className="space-y-4">
            ...
            {/* Góc nhìn说明 */}
            <div className="space-y-2">
              <Label className="text-xs">Góc nhìn布局 (2x2)</Label>
            ...
            {/* Ảnh tham chiếuXem trước（Tự động获取） */}
            {(() => {
              ...
              if (selectedScene?.parentSceneId) {
                const { scenes } = useSceneStore.getState();
                // 查找同一父Cảnh的Tất cảconCảnh，找 viewpointId='overview' 的那
                const overviewScene = scenes.find(s => 
                  s.parentSceneId === selectedScene.parentSceneId && 
                  (s as any).viewpointId === 'overview'
                );
              ...
              // 2. 当前conCảnhảnh
              if (selectedScene?.referenceImage && selectedScene.referenceImage !== overviewImage) {
                referenceImages.push({ label: '当前Góc nhìn', src: selectedScene.referenceImage });
              } else if (selectedScene?.referenceImageBase64 && selectedScene.referenceImageBase64 !== overviewImage) {
                referenceImages.push({ label: '当前Góc nhìn', src: selectedScene.referenceImageBase64 });
              }
              ...
              return (
                <div className="space-y-2">
                  <Label className="text-xs">Ảnh tham chiếu（Tự động获取）</Label>
                  <div className="grid grid-cols-2 gap-2">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    💡 使用「Toàn cảnh」conCảnh作为主Tham chiếu，确保四góc nhìnPhong cách一致
                  </p>
                </div>
              );
            })()}
              ...
              <div className="space-y-2">
                ...
                <label className="block">
                  ...
                  <div className="flex items-center justify-center gap-2 p-2 border border-dashed rounded-lg cursor-pointer hover:border-primary hover:bg-muted/50 transition-colors">
                    ...
                    <span className="text-xs text-muted-foreground">Tải lên已有ảnh</span>
                  </div>
                </label>
              </div>
            )}

            {/* prompt（Mặc địnhMở rộng，可Chỉnh sửa，根据语言偏好只Hiện一种） */}
            <details className="group" open>
              <summary className="flex items-center gap-1 text-xs text-muted-foreground cursor-pointer hover:text-foreground">
                ...
                四góc nhìnprompt（可Chỉnh sửa，修改后Trực tiếp用于Tạo）
              </summary>
                  ...
                  return (
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        <Label className="text-xs">Tạoprompt（{langLabel}，修改后Trực tiếp用于Tạo）</Label>
                        <Button
                          variant="ghost"
                          size="sm"
            ...
            {orthographicImage && (
              <div className="space-y-2">
                <Label className="text-xs">四góc nhìnXem trước ({orthographicAspectRatio})</Label>
  ...
  if (contactSheetPrompt) {
    ...
    return (
      <div className="h-full flex flex-col">
        <div className="p-3 pb-2 border-b flex items-center justify-between">
          <div className="flex items-center gap-2">
            <h3 className="font-medium text-sm">多Góc nhìn联合图</h3>
            {hasMultiplePages && (
        ...
        <ScrollArea className="flex-1 p-3">
          <div className="space-y-4">
            ...
            {/* Phong cách thị giác + Tỷ lệ khung hình + 布局Chọn */}
            <div className="space-y-2">
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-2">
                  <Label className="text-xs">Phong cách thị giác</Label>
              ...
              <div className="space-y-2">
                ...
                <p className="text-[10px] text-muted-foreground">
                  {(() => {
                    const dims = getLayoutDimensions(contactSheetLayout, contactSheetAspectRatio);
                    return `${dims.rows} hàng${dims.cols} cột = ${dims.rows * dims.cols}格`;
                  })()}
                </p>
              </div>
            </div>
            
            {/* Danh sách góc nhìn（Hiện关联Phân cảnh序号） */}
            <div className="space-y-2">
              <Label className="text-xs">
                当前页Góc nhìn ({currentPageViewpointsWithIndexes.length > 0 ? currentPageViewpointsWithIndexes.length : extractedViewpoints.length})
              </Label>
            ...
            <details className="group" open>
              <summary className="flex items-center gap-1 text-xs text-muted-foreground cursor-pointer hover:text-foreground">
                <span className="group-open:rotate-90 transition-transform">▶</span>
                联合图prompt（可Chỉnh sửa，修改后Trực tiếp用于Tạo）
              </summary>
              <div className="mt-2 space-y-2">
                {(() => {
                  const effectiveLang = promptLanguage || scriptProject?.promptLanguage || 'vi';
            ...
            {Object.keys(splitViewpointImages).length > 0 && (() => {
              ...
              // 根据Tỷ lệ khung hình决定切割结果的HiệnTỷ lệ
        ...
        <div className="p-3 border-t">
          <p className="text-xs text-muted-foreground text-center">
            💡 Nhấp「Tạo联合图」后Tự động完成切割和Lưu，可连续发起多任务
          </p>
        </div>
      </div>
    );
    ...
    <div className="h-full flex flex-col">
      <div className="p-3 pb-2 border-b space-y-2">
        ...
        >
          ...
          <ToggleGroupItem value="orthographic" aria-label="四góc nhìn" className="text-xs px-2.5 h-7 data-[state=on]:bg-primary data-[state=on]:text-primary-foreground">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="详细Mô tảCảnh的môi trường，例如：繁华的东京涩谷十字路口，霓虹灯闪烁..."
          ...
          <div className="space-y-2">
            ...
            <p className="text-[10px] text-muted-foreground">
              AI 将Tham chiếu这些Tạo ảnhCảnh概念图
            </p>
          </div>
        </div>
      </ScrollArea>
      ...
      <div className="p-3 border-t space-y-2">
        {/* 批量四góc nhìnnút（在Lưu联合图Góc nhìn后Hiện） */}
        {savedChildSceneIds.length > 0 && (
          <div className="p-3 rounded-lg border-2 border-dashed border-primary/50 bg-primary/5 space-y-2">
            <div className="text-xs text-center">
              ...
              <p className="text-muted-foreground">可为每conCTạo ảnh四góc nhìn（共 {savedChildSceneIds.length * 4} 张）</p>
            </div>
            ...
            >
              {isGenerating ? (
                ...
                <>
                  ...
                  {selectedScene.referenceImage ? 'Tạo lại概念图' : 'TạoCảnh概念图'}
                </>
              )}
            </Button>
          )
        ...
        {/* 联合图chế độ - 无论是否đã chọnCảnh都HiệnTải lênTùy chọn */}
        {generationMode === 'contact-sheet' && (
          <div className="space-y-2">
            ...
            <label className="block">
              ...
              <div className="flex items-center justify-center gap-2 p-2 border border-dashed rounded-lg cursor-pointer hover:border-primary hover:bg-muted/50 transition-colors">
                <Upload className="h-3 w-3 text-muted-foreground" />
                <span className="text-xs text-muted-foreground">Trực tiếpTải lên联合图切割</span>
              </div>
            </label>
          </div>
        )}
        ...
        <p className="text-xs text-muted-foreground text-center">
          {generationMode === 'single' && '💡 单图chế độ：Tạo单一Góc nhìn的Cảnh概念图'}
          {generationMode === 'contact-sheet' && '💡 联合图chế độ：Tạo 2x3 多Góc nhìnCảnh网格'}
          {generationMode === 'orthographic' && '💡 四góc nhìnchế độ：Tạo前/后/左/右正交Góc nhìn'}
        </p>
      </div>
    </div>
  );
...
function extractPropsFromActions(actions: string): string[] {
  ...
  // 常见Đạo cụquan trọng词映射（đang xử lý...> 英文）
  ...
  // 检查每quan trọng词是否出现在Hành động描写中
