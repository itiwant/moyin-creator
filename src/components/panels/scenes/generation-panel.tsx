...
Phong cách: ${stylePreset?.name || 'Phong cách hoạt hình'}, ánh sáng các ô đồng nhất, các ô ngăn cách bằng viền trắng mỏng, chỉ có nền, không có nhân vật.`;
  ...
  /**
   * Sao chépprompt（包含Phong cách thị giác和Tỷ lệ khung hìnhthông tin）
   */
  ...
  const handleDirectUploadContactSheet = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // TạoMặc định的prompt页面dữ liệu（用于切割时获取bố cụcthông tin）
  ...
  const handleAutoGenerateContactSheet = async () => {
    ...
    (async () => {
      try {
        ...
        console.log('[AutoContactSheet] 阶段1hoàn thành，ảnhURLLoại:', 
        ...
        // ==================== hoàn thành ====================
        console.log('[AutoContactSheet] ✅ 流水线hoàn thành:', {
          parentSceneId,
...
No characters, empty environment.`;
        ...
        successCount++;
        console.log(`[批量四góc nhìn] ${childScene.name} hoàn thành`);
      } catch (err) {
        failCount++;
    ...
    setSavedChildSceneIds([]);
    toast.success(`批量四góc nhìnhoàn thành！Thành công ${successCount} ，Thất bại ${failCount} `);
  };
  ...
  if (contactSheetPrompt) {
    ...
    return (
      <div className="h-full flex flex-col">
        ...
        <div className="p-3 border-t">
          <p className="text-xs text-muted-foreground text-center">
            💡 Nhấp「Tạo联合图」后Tự độnghoàn thành切割和Lưu，可连续发起多任务
          </p>
        </div>
      </div>
    );
