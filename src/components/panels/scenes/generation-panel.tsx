...
export function GenerationPanel({ selectedScene, onSceneCreated }: GenerationPanelProps) {
  ...
  useEffect(() => {
    ...
    if (data.name && data.location) {
      ...
      // Tự độngTạoCảnh（包含chuyên nghiệpThiết kếtrường）
      ...
      // 如果有多Góc nhìndữ liệu，Trực tiếp进入联合图Tạochế độ
...
Phong cách: ${stylePreset?.name || 'Phong cách hoạt hình'}, ánh sáng các ô đồng nhất, các ô ngăn cách bằng viền trắng mỏng, chỉ có nền, không có nhân vật.`;
  ...
  const handleGenerateContactSheetImage = async () => {
    ...
    try {
      ...
      // 如果Quay lại的是 HTTP URL，转为 base64 — Tránh后续切割时 CORS 问题
  ...
  const handleDirectUploadContactSheet = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // TạoMặc định的prompt页面dữ liệu（用于切割时获取bố cục信息）
    ...
    (async () => {
      try {
        ...
        for (const vp of viewpointsToSave) {
          ...
          if (!imgData) {
            console.warn(`[AutoContactSheet] 跳过Góc nhìn ${vp.name}: viewpointImagesMap đang xử lý...dữ liệu (id=${vp.id})`);
            continue;
          }
...
No characters, empty environment.`;
  ...
  // ========== 四góc nhìn（正交góc nhìn）chức năng ==========
...
Unified by flat, neutral cinematic lighting to ensure texture visibility. No characters, empty environment.`;
    ...
    // prompt tiếng Trung
    const promptZh = `chuyên nghiệp正交概念图，精确的 2x2 网格排 cột，Hiển thị「${sceneName}」的四基本Góc nhìn，保持完美的空间连续性。${stylePreset?.name || 'Phong cách hoạt hình'}，详细的môi trường概念艺术。
...
Sử dụng平坦、đang xử lý...影ánh sáng以确保纹理可见。无Nhân vật，空Cảnh。`;
  ...
  if (contactSheetPrompt) {
    ...
    // 获取当前页的Góc nhìndữ liệu（带Phân cảnhsố thứ tự）
