...
export function GenerationPanel({ selectedScene, onSceneCreated }: GenerationPanelProps) {
  ...
  useEffect(() => {
    ...
    if (data.name && data.location) {
      ...
      if (data.viewpoints && data.viewpoints.length > 0 &&
        ...
        // 同步布局Cài đặt，确保切割时Sử dụng正确的 hàng cột数
  ...
  useEffect(() => {
    ...
    // 强制Sử dụng N x N 布局以保证Tỷ lệ khung hình一致性
    let newLayout: { rows: number; cols: number };
    
    // 如果Góc nhìn数量 <= 4，Sử dụng 2x2
    // 如果Góc nhìn数量 > 4，Sử dụng 3x3
    if (vpCount <= 4) {
...
Phong cách：${stylePreset?.name || 'Phong cách hoạt hình'}，Tất cảôánh sáng一致，ô之间用细白边框ngăn cách，chỉ có nền，không có nhân vật。`;
   ...
   * Sử dụng submitGridImageRequest 对齐Đạo diễnpanel，确保网格格式正确
   */
  ...
  const handleSplitContactSheet = async () => {
    ...
    try {
      ...
      // 如果没有，才Sử dụng用户Chọn的 contactSheetLayout
      ...
      if (currentPagePrompt?.gridLayout) {
        ...
        console.log('[Split] Sử dụng用户Chọn的布局:', { expectedRows, expectedCols, contactSheetLayout });
      }
  ...
  const handleSaveViewpointImages = async () => {
    ...
    if (sceneShots.length > 0) {
      ...
      if (unassignedShots.length > 0) {
        console.log(`[ContactSheet] Phát hiện ${unassignedShots.length} 未分配的Phân cảnh，按序号分配到Góc nhìn`);
    ...
    for (const vp of viewpointsToUse) {
      ...
      if (!localPath.startsWith('local-image://')) {
        console.warn(`[ContactSheet] Góc nhìnảnh本地LưuThất bại: ${vp.name}, 将Sử dụnggốc URL`);
      }
  ...
  const handleAutoGenerateContactSheet = async () => {
    ...
    (async () => {
      try {
        // ==================== 阶段 1: Tạo联合图 ====================
        // 获取 API 配置 — 与Đạo diễnpanel一致Sử dụng submitGridImageRequest
        ...
        // Sử dụng submitGridImageRequest — 与Đạo diễnpanel保持一致
        const result = await submitGridImageRequest({
          model,
          prompt: finalPrompt,
          apiKey,
        ...
        console.log('[AutoContactSheet] 切割完成，kết quả数量:', splitResults.length);

        // 如果 snapshotViewpoints 为空（用户Thủ côngChỉnh sửaprompt，未走Góc nhìnTạo流程），
        // Tự độngTạo fallback Góc nhìn以匹配切割kết quả
        ...
        // 将切割kết quả映射到Góc nhìn — 双重映射策略：优先Trực tiếp索引，回退到 row/col 查找
        const viewpointImagesMap: Record<string, { imageUrl: string; gridIndex: number }> = {};
        for (const vp of effectiveViewpoints) {
          ...
          if (splitResult) {
            ...
            console.warn(`[AutoContactSheet] Góc nhìn ${vp.name}(gridIndex=${gridIdx}) 未找到对应切割kết quả`);
          }
        }
        ...
        // ===== 安全回退：如果映射Tất cảThất bại但切割有kết quả，Trực tiếpSử dụng切割kết quảTạoconCảnh =====
        if (mappedCount === 0 && splitResults.length > 0) {
          console.warn('[AutoContactSheet] ⚠ 映射Tất cảThất bại！Bật安全回退：Trực tiếpSử dụng切割kết quảTạoconCảnh');
        ...
        if (createdVariantIds.length > 0) {
          toast.success(`Cảnh「${parentSceneName}」联合图已切割Lưu，共 ${createdVariantIds.length} Góc nhìnconCảnh（NhấpMở rộngXem）`);
        } else {
          toast.warning(`Cảnh「${parentSceneName}」联合图đã lưu，但未能TạoconCảnh（切割kết quả: ${splitResults.length} ）`);
        }
...
No characters, empty environment.`;
        ...
        if (overviewImage) {
          ...
          console.log(`[批量四góc nhìn] ${childScene.name} Sử dụngToàn cảnhconCảnh作为Tham chiếu`);
        }
...
Sử dụng平坦、đang xử lý...影ánh sáng以确保纹理可见。无Nhân vật，空Cảnh。`;
  ...
  const handleGenerateOrthographicImage = async () => {
    ...
    try {
      ...
      // 收 tậpẢnh tham chiếu：优先Sử dụng「Toàn cảnh」conCảnh，而不是整张联合图
      ...
      if (overviewImage) {
        ...
        console.log('[Orthographic] Sử dụngToàn cảnhconCảnh作为主Tham chiếu');
      }
  ...
  const handleSplitOrthographic = async () => {
    ...
    try {
      ...
      const splitResults = await splitStoryboardImage(orthographicImage, {
        aspectRatio: orthographicAspectRatio, // Sử dụng用户Chọn的Tỷ lệ khung hình
        resolution: '2K',
        sceneCount: 4,
        options: {
          expectedRows: 2,
              ...
              return (
                <div className="space-y-2">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    💡 Sử dụng「Toàn cảnh」conCảnh作为主Tham chiếu，确保四góc nhìnPhong cách一致
                  </p>
                </div>
              );
            })()}
        ...
        <ScrollArea className="flex-1 p-3">
          <div className="space-y-4">
            ...
            {Object.keys(splitViewpointImages).length > 0 && (() => {
              ...
              // 根据Tỷ lệ khung hình决定切割kết quả的HiệnTỷ lệ
