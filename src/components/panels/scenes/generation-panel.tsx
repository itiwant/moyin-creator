...
Phong cách：${stylePreset?.name || 'Phong cách hoạt hình'}，Tất cảôánh sáng一致，ô之间用细白边框ngăn cách，chỉ có nền，không có nhân vật。`;
   ...
   * Sử dụng submitGridImageRequest 对齐Đạo diễn面板，确保网格格式正确
   */
  ...
  const handleAutoGenerateContactSheet = async () => {
    ...
    (async () => {
      try {
        // ==================== 阶段 1: Tạo联合图 ====================
        // 获取 API 配置 — 与Đạo diễn面板一致Sử dụng submitGridImageRequest
        ...
        // Sử dụng submitGridImageRequest — 与Đạo diễn面板保持一致
        const result = await submitGridImageRequest({
          model,
          prompt: finalPrompt,
          apiKey,
