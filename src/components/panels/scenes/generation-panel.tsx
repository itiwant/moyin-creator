...
export function GenerationPanel({ selectedScene, onSceneCreated }: GenerationPanelProps) {
  ...
  // 当người dùngthay đổiTỷ lệ khung hình时，根据Góc nhìn数量lại计算最优bố cục
...
Phong cách: ${stylePreset?.name || 'Phong cách hoạt hình'}, ánh sáng các ô đồng nhất, các ô ngăn cách bằng viền trắng mỏng, chỉ có nền, không có nhân vật.`;
  ...
  /**
   * độc lậpTải lên联合图入sổ（不需要先Tạoprompt）
  ...
  const handleSaveViewpointImages = async () => {
    ...
    // 仅Lưu新Tạo的conCảnh ID，用于hàng loạt四góc nhìn
  ...
  /**
   * 清除hàng loạt四góc nhìnTrạng thái
   */
...
No characters, empty environment.`;
        ...
        if (overviewImage) {
          ...
          console.log(`[hàng loạt四góc nhìn] ${childScene.name} Sử dụngToàn cảnhconCảnh作为Tham chiếu`);
        }
        ...
        successCount++;
        console.log(`[hàng loạt四góc nhìn] ${childScene.name} hoàn thành`);
      } catch (err) {
        failCount++;
        console.error(`[hàng loạt四góc nhìn] ${childScene.name} Thất bại:`, err);
      }
    }

    setSavedChildSceneIds([]);
    toast.success(`hàng loạt四góc nhìnhoàn thành！Thành công ${successCount} ，Thất bại ${failCount} `);
  };
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="p-3 border-t space-y-2">
        {/* hàng loạt四góc nhìnnút（在Lưu联合图Góc nhìn后Hiện） */}
