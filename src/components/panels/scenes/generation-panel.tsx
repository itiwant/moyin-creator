  ...
  const handleSaveViewpointImages = async () => {
    ...
    if (sceneShots.length > 0) {
      ...
      if (unassignedShots.length > 0) {
        console.log(`[ContactSheet] Phát hiện ${unassignedShots.length} 未分配的Phân cảnh，按số thứ tự分配到Góc nhìn`);
        
        // 按Phân cảnhsố thứ tự分配到Góc nhìn（Phân cảnh1->Góc nhìn1，Phân cảnh2->Góc nhìn2，...）
  ...
  const handleAutoGenerateContactSheet = async () => {
    ...
    (async () => {
      try {
        ...
        // Tự độngTạo fallback Góc nhìn以Khớp切割kết quả
        ...
        // 将切割kết quả映射到Góc nhìn — 双重映射策略：优先Trực tiếpchỉ mục，回退到 row/col 查找
        const viewpointImagesMap: Record<string, { imageUrl: string; gridIndex: number }> = {};
        for (const vp of effectiveViewpoints) {
          ...
          if (splitResult) {
            ...
            if (splitResult.row !== expectRow || splitResult.col !== expectCol) {
              console.warn(`[AutoContactSheet] Trực tiếpchỉ mục不Khớp: gridIndex=${gridIdx}, split[row=${splitResult.row},col=${splitResult.col}] vs expected[row=${expectRow},col=${expectCol}]`);
              splitResult = undefined; // 不Khớp，回退到查找
            }
          }
          // 策略 2: row/col 查找
          if (!splitResult) {
...
**右下（bên phảigóc nhìn）：**
从bên phải拍摄的 ${anchor} bên cạnhỐng kính。背景是对面的墙壁，严格包含 ${walls.west}。
...
Sử dụng平坦、đang xử lý...影ánh sáng以确保纹理可见。无Nhân vật，空Cảnh。`;
  ...
  const handleSplitOrthographic = async () => {
    ...
    try {
      ...
      // 映射到四Góc nhìn: 左上=chính diện, 右上=mặt sau, 左下=左侧, 右下=bên phải
  ...
  if (contactSheetPrompt) {
    ...
    // 获取当前页的Góc nhìn数据（带Phân cảnhsố thứ tự）
      ...
      <div className="h-full flex flex-col">
        ...
        <ScrollArea className="flex-1 p-3">
          <div className="space-y-4">
            ...
            {/* Danh sách góc nhìn（Hiện关联Phân cảnhsố thứ tự） */}
            <div className="space-y-2">
