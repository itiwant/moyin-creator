  ...
  if (shots.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="text-sm text-zinc-500">Chưa có phân cảnh</p>
        <p className="text-xs text-zinc-600 mt-1">Sau khi Phân tích kịch bản, nhấp "Tạo Danh sách phân cảnh"</p>
      </div>
    );
  }
    ...
    >
      ...
      <div className="flex items-center justify-between mb-1">
        ...
        <div className="flex items-center gap-1">
          {shot.imageUrl && (
            <span className="w-2 h-2 rounded-full bg-green-500" title="Hình ảnh đã tạo" />
          )}
          {shot.videoUrl && (
            <span className="w-2 h-2 rounded-full bg-blue-500" title="Video đã tạo" />
          )}
        </div>
      </div>
