...
格式：${s2JsonFormat}`;
  ...
  const s3System = `你是电影摄影指导(DP)。根据Mô tả thị giác确定专业拍摄参数。${cinematographyGuidance ? `\n\n${cinematographyGuidance}` : ''}
...
格式：{"shots":{"shot_id":{...}}}`;
  ...
  const s4System = `你是AI图像Tạo专家。根据Mô tả thị giác和拍摄参数，Tạo首帧提示词。${eraContextBlock}
...
格式：${s4JsonFormat}`;
  ...
  const s5System = `你是AI视频Tạo专家。根据首帧画面，Tạo视频动作Mô tả和尾帧画面。${eraContextBlock}
...
- Mô tả动作完成后的最终画面
- 包含与首帧相同的场景环境和光线
- 重点Mô tả与首帧的差异（新位置/新Tư thế/新Biểu cảm/道具新状态）
