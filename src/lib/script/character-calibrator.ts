...
export async function calibrateCharacters(
  ...
  // 2. 构建带统计信息的角色列表，按thông minh优先级排序
...
- 只lọc纯Quần chúng、群体、非角色词`;
  ...
  const systemPrompt = `你是专业的影视剧本分析师，擅长从剧本数据đang xử lý...Hiệu chuẩn角色。${seriesCtxBlock}
【核心目标】
...
3. 不要TạoQuần chúngXX组标签`;
  ...
  // === 第三步：为nhân vật chính和重要nhân vật phụTạo专业视觉提示词（独立 try/catch，thất bại不影响Hiệu chuẩnkết quả）===
  ...
  try {
    ...
    console.warn('[CharacterCalibrator] 视觉提示词Tạothất bại（不影响角色Hiệu chuẩnkết quả）:', err.message);
...
export function convertToScriptCharacters(
  ...
  return calibrated.map(c => {
    ...
    // 合并：保留gốc数据，只补充/更新 AI Tạo的trường
    return {
      // 保留gốctrường
      ...original,
...
/**
 * 为nhân vật chính和重要nhân vật phụTạo专业的视觉提示词
...
绝对禁止出现与该thời đại不符的服装元素。`;
  ...
  // 系统提示词：角色Thiết kế大师 + 背景信息 + 输出格式（不含具体角色）
...
${promptLanguage === 'zh' ? `    "avoid": ["金色头发", "蓝色眼睛", "胡须", "纹身"],
  ...
  for (let i = 0; i < keyCharacters.length; i++) {
    ...
    const userPrompt = `请为以下角色Tạo专业视觉提示词和6层身份neo：
...
- 出场：${c.appearanceCount}次`;
  ...
  console.log(`[enrichCharactersWithVisualPrompts] 完成: ${designMap.size}/${keyCharacters.length} 角色Tạo成功`);
