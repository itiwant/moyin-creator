...
3. 不要TạoQuần chúngXX组标签`;
  ...
  // === 第三步：为nhân vật chính和重要nhân vật phụTạochuyên nghiệp视觉提示词（独立 try/catch，thất bại不影响Hiệu chuẩnkết quả）===
...
/**
 * 为nhân vật chính和重要nhân vật phụTạochuyên nghiệp的视觉提示词
...
- 老年人：đang xử lý...棉袄、布鞋`;
        ...
        return `【${startYear}年代trang phục指导】
请根据该年代的đang xử lý...trang phục风格Thiết kế，Tránh古装或不符合thời đại的trang phục`;
      }
    }
...
${promptLanguage === 'zh' ? `    "avoid": ["金色头发", "蓝色眼睛", "胡须", "纹身"],
  ...
  for (let i = 0; i < keyCharacters.length; i++) {
    ...
    const userPrompt = `请为以下角色Tạochuyên nghiệp视觉提示词和6层身份neo：
