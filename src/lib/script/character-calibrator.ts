 ...
 * 
 * 使用 AI 智能校准从剧本đang xử lý...角色列表
 * 
 * 功能：
 * 1. 统计每角色的出场次数、对白条数、出场集数
 * 2. AI 分析识别真正角色 vs 非角色词
 * 3. AI 合并重复角色（王总 = 投资人王总）
 * 4. AI 分类主角/配角/龙套（结合出场统计）
 * 5. AI 补充角色信息（年龄、Giới tính、关系）
 */
 ...
 * 这会遍历Tất cả集的Tất cả场景，提取场景nhân vật和对白说话人
 */
export function extractAllCharactersFromEpisodes(
  ...
  console.log(`[extractAllCharactersFromEpisodes] 从 ${episodeScripts.length} 集剧本đang xử lý... ${characters.length} 角色`);
  return characters;
}
...
- 群演Tất cả过滤`
    : strictness === 'loose'
    ? `【筛选chế độ：宽松】
- 几乎不过滤，保留Tất cả能识别的角色
- 包括群演、低频角色、只有称呼的角色（如"学习委员"、"戴眼镜的男生"）
- 只过滤纯描述词（如"眼框微湿"、"干练优雅"）和非nhân vật词（如"全体员工"、"核心团队"）`
    : `【筛选chế độ：标准】
- 有名字或称呼的角色Tất cả保留
- 只过滤纯群演、群体、非角色词`;
  ...
  const systemPrompt = `你是专业的影视剧本分析师，擅长从剧本数据đang xử lý...校准角色。${seriesCtxBlock}
【核心目标】
校准后的角色列表将用于生成角色三góc nhìn。
...
【数量约束】
- 主角：1-3 
- 配角：5-30 （有名字的Tất cả保留，不要限制）
- 总角色数：建议 15-40 ，宁多勿少

【重要】每被过滤的角色请在 filteredCharacters đang xử lý...滤原因。
...
【极其重要！请特别注意】
1. ${strictness === 'strict' ? '严格过滤低频无名角色' : strictness === 'loose' ? '尽可能保留Tất cả角色，包括群演' : '有名字的Tất cả保留！有称呼的Tất cả保留！不确定的保留！'}
2. 每被过滤的角色必须在 filteredCharacters đang xử lý...因
3. 不要生成群演XX组标签`;
    ...
    console.log('[CharacterCalibrator] AI 角色分析成功，解析到', parsed.characters.length, '角色');
  } catch (error) {
...
function collectCharacterContexts(
  ...
  for (const ep of episodeScripts.slice(0, 5)) { // 只取前5集作为样本
    for (const scene of ep.scenes.slice(0, 10)) { // 每集最多10场景
 ...
 * 注意：保留原始角色的Tất cảtrường，只补充/更新 AI 校准的trường
 */
...
async function enrichCharactersWithVisualPrompts(
  ...
  console.log(`[enrichCharactersWithVisualPrompts] 为 ${keyCharacters.length} quan trọng角色生成专业提示词...`);
...
- 老年人：đang xử lý...棉袄、布鞋`;
        ...
        return `【${startYear}年代服装指导】
请根据该年代的đang xử lý...服装风格设计，避免古装或不符合时代的服装`;
      }
    }
    ...
    if (timeline.includes('现代') || timeline.includes('当代')) {
      return `【现代服装指导】
请设计符合当代đang xử lý...装风格，年轻人穿时尚休闲装，đang xử lý...商务休闲装，老年人穿舒适传统服装。
绝对不要设计成古装、汉服、或古代服饰。`;
    }
...
- 男性：长衫马褂、đang xử lý...西装礼帽（上层社会）、布衣长衫（平民）
...
${promptLanguage === 'zh' ? `【核心输出：6层身份neo】
...
⑥ 发型neo层
   - hairStyle: 发型，用đang xử lý...（齐肩层次剪、寸头、波波头）
   - hairlineDetails: 发际线，用đang xử lý...（自然发际线、美人尖、额角后退）

【负面提示词】
为角色生成negativePrompt，排除不符合设定的特征，用đang xử lý...：
- avoid: 要避免的特征（如đang xử lý...色应避免 金色头发、蓝色眼睛）
- styleExclusions: 风格排除（如 动漫风、卡通风、油画风）` : `【核心输出：6层身份neo】
...
${promptLanguage === 'zh' ? `    "avoid": ["金色头发", "蓝色眼睛", "胡须", "纹身"],
  ...
  // 逐角色调用 AI，避免一次性输出过多 JSON 导致推理模型 token 耗尽
  ...
  for (let i = 0; i < keyCharacters.length; i++) {
    ...
    const userPrompt = `请为以下角色生成专业视觉提示词和6层身份neo：
...
- 出场：${c.appearanceCount}次`;
  ...
  console.log(`[enrichCharactersWithVisualPrompts] 完成: ${designMap.size}/${keyCharacters.length} 角色生成成功`);
  ...
  return characters.map(c => {
    ...
    if (design) {
      ...
      // 从新的 identityAnchors đang xử lý...ương thíchtrường（根据neo值语言自动适配标签）
