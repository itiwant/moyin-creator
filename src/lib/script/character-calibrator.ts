...
3. 不要生成群演XX组标签`;
  ...
  // === 第三步：为nhân vật chính和重要nhân vật phụ生成专业视觉提示词（独立 try/catch，失败不影响校准结果）===
 ...
 * 注意：保留gốc角色的Tất cảtrường，只补充/更新 AI 校准的trường
 */
export function convertToScriptCharacters(
  ...
  return calibrated.map(c => {
    ...
    // 合并：保留gốc数据，只补充/更新 AI 生成的trường
    return {
      // 保留gốctrường
      ...original,
      // 更新/补充 AI 校准的trường
...
/**
 * 为nhân vật chính和重要nhân vật phụ生成专业的视觉提示词
 * gọi API世界级角色设计大师 AI
 */
async function enrichCharactersWithVisualPrompts(
  ...
  // 只为nhân vật chính和重要nhân vật phụ生成详细提示词
...
- 老年人：đang xử lý...棉袄、布鞋`;
        ...
        return `【${startYear}年代服装指导】
请根据该年代的đang xử lý...服装风格设计，避免古装或不符合thời đại的服装`;
      }
    }
...
- 禁止出现与Bối cảnh thế giới不符的服装元素`;
    ...
    return `【${timeline}服装指导】
请根据「${timeline}」thời đại背景设计角色服装，服装、发型、配饰必须严格符合该thời đại特征。
...
- 根据角色年龄和身份设计合适的服装
- 绝对不要设计与剧本thời đại不符的服饰（如古装剧禁止现代服装，现代剧禁止古代服饰）
...
${promptLanguage === 'zh' ? `    "avoid": ["金色头发", "蓝色眼睛", "胡须", "纹身"],
  ...
  // 逐角色gọi API AI，避免一次性输出过多 JSON 导致推理模型 token 耗尽
...
- 出场：${c.appearanceCount}次`;
  ...
  return characters.map(c => {
    ...
    if (design) {
      ...
      // 从新的 identityAnchors đang xử lý...ương thíchtrường（根据neo值Ngôn ngữ自动适配标签）
