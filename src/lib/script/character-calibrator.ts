...
export interface CharacterCalibrationResult {
  ...
  /** bị lọc的角色（带原因，用于người dùng确认/恢复） */
...
export function extractAllCharactersFromEpisodes(
  ...
  // chuyển đổi thành ScriptCharacter 数组
...
export async function calibrateCharacters(
  ...
  const charsWithStats = rawCharacters.map(c => {
    ...
    // 判断是否有具体名字（đang xử lý...2-4字，hoặc有昵称后缀）
  ...
  // 限制发送给 AI 的角色số lượng，TránhĐầu racắt ngắn
  // 优先保留有名字的角色
  ...
  // 3. chuẩn bị批处理 items（每角色带上统计thông tin和Thoại样本）
...
**2. 重要nhân vật phụ (supporting)** - 必须保留
   - 有具体名字hoặc昵称：刀疑哥、龙哥、李强、王艳、小乐、阿强
   - 有Cố định称呼：赖董、王总、周总、李医生
   - 出场 ≥1 且有Thoại、hoặc出场 ≥2
...
请按照分级规则Hiệu chuẩn角色，返回JSONđịnh dạng：
{
  "characters": [
    {
      "name": "角色名",
...
3. 不要TạoQuần chúngXX组标签`;
  ...
  // === 第二步：chuyển đổi thànhTiêu chuẩnđịnh dạng并ThêmID ===
  ...
  // === 第4步：合并上次Hiệu chuẩnkết quả，防止角色丢失 ===
...
/**
 * 将Hiệu chuẩnkết quảchuyển đổi回 ScriptCharacter định dạng
...
- 老年人：đang xử lý...简单开衫、布鞋`;
        ...
        return `【${startYear}thập niêntrang phục指导】
- 年轻人：喝叭裤、确良外套、大肩垫Tây装、特宾球鞋
- đang xử lý...đang xử lý...Tây装，常穿解放鞋hoặc简单皮鞋
- 老年人：đang xử lý...棉袄、布鞋`;
      } else {
        return `【${startYear}thập niêntrang phục指导】
请根据该thập niên的đang xử lý...trang phục风格Thiết kế，Tránh古装hoặc不符合thời đại的trang phục`;
      }
    }
...
绝对不要Thiết kế成古装、汉服、hoặc古代服饰。`;
    }

    // 民国时期
    if (timeline.includes('民国') || timeline.includes('近代') || timeline.includes('清末')) {
      return `【${timeline}trang phục指导】
- 男性：长衫马褂、đang xử lý...Tây装礼帽（上层xã hội）、布衣长衫（平民）
- 女性：旗袍、女学生装（上衣下裙）、短发hoặc盘发
...
- 绝对bị cấm任何现代trang phục（Tây装/T恤/牵仔裤/运动鞋）`;
    }
...
- 女性：旗装（溜肩、立领、Lỏng lẻo）、旗头hoặc两把头
- 绝对bị cấm任何现代trang phục`;
    }
...
- 绝对bị cấm任何现代trang phục（Tây装/T恤/牛仔裤/运动鞋/手机/眼镜等）`;
    }

    // 科幻/未来
    if (/科幻|未来|星际|太空/.test(timeline)) {
      return `【${timeline}trang phục指导】
- 可以Thiết kế未来感/科技感trang phục，但需giữ内部giống性
...
绝对bị cấm出现与该thời đại不符的trang phục元素。`;
  ...
  // 系统提示词：角色Thiết kế大师 + 背景thông tin + Đầu rađịnh dạng（不含具体角色）
...
- **角色giống性专家**：掌握"6层特征锁定"技术，确保同一角色在不同场景giữgiống
...
${promptLanguage === 'zh' ? `【核心Đầu ra：6层Danh tínhneo】
...
- styleExclusions: 风格排除（如 动漫风、卡通风、油画风）` : `【核心Đầu ra：6层Danh tínhneo】
...
请返回JSONđịnh dạng（注意：只返回单角色对象，不要数组包裹）：
{
  "name": "角色名",
...
${promptLanguage === 'zh' ? `    "avoid": ["金色头发", "蓝色眼睛", "胡须", "纹身"],
  ...
  // 逐角色gọi API AI，Tránh一次性Đầu ra过多 JSON 导致推理模型 token 耗尽
...
- 出场：${c.appearanceCount}次`;
    ...
    try {
      const result = await callFeatureAPI('script_analysis', systemPrompt, userPrompt, {
        maxTokens: 4096, // 单角色Đầu ra 4096 足够
      });
      ...
      // tương thích：AI 可能返回 { characters: [...] } hoặcTrực tiếp返回单角色对象
  ...
  return characters.map(c => {
    ...
    if (design) {
      ...
      // uniqueMarks 从 anchors.uniqueMarks 数组chuyển đổi thànhký tự串（向后tương thích）
