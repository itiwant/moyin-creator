  ...
  "scenes": [
    {
      ...
      "location": "详细地点描述（包括建筑特征、环境元素、địa lý特点等）",
...
**nhân vật小传：**
角色A：[XX tuổi]，[身份/职业]，[性格特点]，[Đặc điểm ngoại hình]
角色B：[XX tuổi]，[身份/职业]，[性格特点]，[Đặc điểm ngoại hình]
...
9. **Bối cảnh thế giới一致性**：场景地点、建筑风格、xã hội规则必须符合剧本设定的Bối cảnh thế giới，不得出现mâu thuẫn元素`;
...
${originalShotCount > 0 ? `- 场景数量：必须有 ${originalShotCount} （与gốc镜头一一对应）` : sceneCount ? `- 场景数量：约 ${sceneCount} ` : '- 场景数量：根据内容和时长自行决定'}
