...
const PARSE_SYSTEM_PROMPT = `你是一专业的剧本分析师。分析用户提供的剧本/故事文本，提取Cấu trúc化信息。
  ...
  "characters": [
    {
      ...
      "traits": "Đặc trưng cốt lõi的详细描述，包括突出能力、特点等",
      "skills": "Kỹ năng/năng lực描述（如武功招式、魔法、专业技能等）",
      "keyActions": "quan trọng行为/事迹描述，重要的历史行动",
  ...
  "scenes": [
    {
      ...
      "atmosphere": "详细氛围描述（如：căng thẳng压抑、ấm cúng宁静、bí ẩn阴森、悲壮肇杀）",
      "visualPrompt": "场景的详细视觉描述，用于生成场景概念图（包括光线、天气、建筑风格、特殊元素等，用英文）",
      "tags": ["场景quan trọng元素标签，如: 木柱, 窗棱, 古建筑, 废墟, 深林"],
...
1. 【角色信息必须详细】：不要简化角色信息！保留原文đang xử lý...t cả细节：
   - role: đầy đủ的身份背景（如"北疆侠义之士，惊鸿剑持有者，曾镇守雁城..."）
   - personality: đầy đủ的性格描述（如"重侠义、护苍生、轻权位、有原则，面对构陷不屑辩解..."）
   ...
   - tags: 角色标签，3-5，描述角色类型和特征（如: 武侠, 男主, 剑客, 守护者）
   - notes: 角色备注，说明这角色在剧情đang xử lý...（如: "本剧主角，第三幕触发冲突"）
2. 【场景设计必须详细】：不要简化场景信息！场景是视觉生成的基础：
   ...
   - tags: 场景quan trọng元素标签，3-6，描述环境特征（如: 木柱, 窗棱, 古建筑, 烟雾, 残垣断壁）
   - notes: 地点备注，说明这场景在剧情đang xử lý...（如: "决战发生的古老殿堂"）
3. 识别多集Cấu trúc。如果剧本包含"第X集"、"Episode X"、"第X章"等标记，要拆分成多 episode
4. 如果没有明确的集标记，创建单 episode 包含Tất cả场景
...
const SHOT_GENERATION_SYSTEM_PROMPT = `你是一专业的分镜师/摄影指导。为单场景生成电影级别的详细镜头列表（Camera Blocking）。
...
[
  {
    ...
    "duration": 4.0,
    "visualDescription": "详细的đang xử lý...描述，包括场景、光线、角色动作、Biểu cảm等",
...
分镜原则：
1. 【重要】每场景最多6-8镜头，避免JSON截断
2. 【景别缩写】WS=远景, MS=đang xử lý...CU=近景, ECU=特写, FS=全景
3. 【镜头运动】使用专业术语：
...
6. 【时长】duration 估算每镜头秒数（2-8秒，根据内容复杂度）
...
export async function callChatAPI(
  ...
  console.log(`[callChatAPI] 使用 ${provider}，共 ${totalKeys}  API keys`);
  ...
  return await retryOperation(async () => {
    ...
    if (!response.ok) {
      ...
      // === Error-driven Discovery: 400 错误自动发现模型限制并Thử lại ===
        ...
        if (discovered) {
          ...
          if (discovered.maxOutput && effectiveMaxTokens > discovered.maxOutput) {
            ...
            console.warn(
              ...
              `以 max_tokens=${correctedMaxTokens} 自动Thử lại...`
            );
    ...
    if (!content) {
      ...
      // 智谱 API 的 sensitive 过滤：尝试xoay vòng key Thử lại
      if (finishReason === 'sensitive' || finishReason === 'content_filter') {
        if (keyManager.handleError(403)) {
          console.warn(`[callChatAPI] 内容被安全过滤(${finishReason})，xoay vòng key Thử lại`);
        }
      ...
      if (finishReason === 'length' && reasoningContent) {
        // 先尝试从 reasoning_content 提取 JSON（少数情况下思考đang xử lý...结果）
        ...
        // 说明模型在「思考」上花了太多预算，以双倍 max_tokens 自动Thử lại一次
        ...
        if (reasoningTokens > 0 && completionTokens > 0 &&
          ...
          console.warn(
            ...
            `以 max_tokens=${newMaxTokens} 自动Thử lại...`
          );
          ...
          if (retryResp.ok) {
            ...
            console.log(
              `[callChatAPI] Thử lại结果: content=${retryContent?.length || 0}字, ` +
...
export async function parseScript(
  ...
  const sceneCountHint = options.sceneCount 
    ? `\n\n【重要】请仅提取最重要的 ${options.sceneCount} 场景，挑选剧情đang xử lý...表性和视觉冲击力的场景。`
    : '';
...
语言：${options.language || 'đang xử lý...${sceneCountHint}`;
...
export async function generateShotList(
  ...
  onShotsGenerated?: (newShots: Shot[], sceneIndex: number) => void // 流式回调，每场景完成后立即通知
): Promise<Shot[]> {
...
=== 分镜要求 ===
1. 为该场景生成${shotsPerScene ? `恰好 ${shotsPerScene} ` : shotsPerSceneHint}镜头，挑选最具视觉冲击力的画面
2. 每镜头必须包含：
   - shotSize: 景别（WS/MS/CU/ECU）
   - duration: 时长（秒）
   - visualDescription: 详细đang xử lý...描述（像写电影剧本那样详细）
...
const CREATIVE_SCRIPT_BASE_PROMPT = `你是一位专业的影视编剧和分镜师。根据用户的创意输入，生成đầy đủ的剧本。
...
**nhân vật小传：**
角色A：[XX岁]，[身份/职业]，[性格特点]，[Đặc điểm ngoại hình]
角色B：[XX岁]，[身份/职业]，[性格特点]，[Đặc điểm ngoại hình]
...
重要要求：
1. 必须包含《标题》、**大纲：**、**nhân vật小传：**、**第X集**
2. 场景头格式：**编号 日/夜 内/外 地点**
3. 每场景必须有"nhân vật："行
4. 动作描写用 △ 开头
5. 对白格式：角色名：（动作）台词
6. MV/广告也要拆分成场景和分镜，只是内容侧重画面和音效
7. 语言与用户输入保持一致（đang xử lý...用đang xử lý...）
8. **时代一致性**：大纲đang xử lý...确时代背景；nhân vật小传đang xử lý...、发型、道具必须严格符合该时代（如古代剧不得出现现代服装/电子产品；现代剧不得出现古代服饰）
...
3. 每原始镜头转换为一 **X-X 日/夜 内/外 地点** 格式的场景
4. 绝对禁止合并、省略、压缩镜头数量
...
2. 一动作行：△[将该镜头Tất cả画面、动作、对白、音效等压缩为一句đầy đủ的视觉描述]

禁止在场景内写多行！禁止分别列出对白、音效！Tất cả内容必须压缩到一 △ 行中。

示例：
用户输入【镜头1】包含画面描述+对白+音效，你的输出应该是：
**1-1 日 内 篮球馆**
nhân vật：马一花、沈星晴
△记分牌特写显示68:70，马一花带球被包夹Biểu cảm焦躁，全场屏息，心跳声逐渐响起
...
export async function generateScriptFromIdea(
  ...
  // 根据时长生成Tham chiếu范围（不是硬限制，是给 AI 的Tham chiếu）
  ...
  if (durationSeconds > 0 && !sceneCount && !shotCount) {
    ...
    durationGuidance = `\n- 时长Tham chiếu：${durationSeconds}秒视频通常包含 ${minShots}-${maxShots} 分镜，请根据内容需要自行把握节奏`;
  }

  // 检测输入类型
  const inputType = detectInputType(idea);
  
  // 统计原始输入đang xử lý.../场景数量
...
重要：每场景内只能有一 △ 动作行！将该镜头的Tất cả画面、对白、音效压缩成一句话。
禁止分别列出多行对白或音效，否则会生成多分镜！`
    : '';
  
  const userPrompt = `请根据以下创意输入生成đầy đủ剧本：
...
${originalShotCount > 0 ? `- 场景数量：必须有 ${originalShotCount} （与原始镜头一一对应）` : sceneCount ? `- 场景数量：约 ${sceneCount} ` : '- 场景数量：根据内容和时长自行决定'}
${originalShotCount > 0 ? '' : shotCount ? `- 分镜数量：约 ${shotCount} ` : '- 分镜数量：根据内容和时长自行决定'}
${styleId ? `- 视觉风格：${styleId}` : ''}

请生成符合标准格式的đầy đủ剧本，包含：
1. 剧本标题
2. 大纲（简述主题/故事）
3. nhân vật小传（每角色的基本信息）
4. đầy đủ的场景和对白${preserveStructureNote}`;
  ...
  // - 有分镜Cấu trúc：使用基础 + 分镜Cấu trúc特殊指令（每场景只能有一动作行）
  // - 无分镜Cấu trúc：使用基础 prompt（允许正常展开多动作/对白）
  const systemPrompt = originalShotCount > 0
    ? CREATIVE_SCRIPT_BASE_PROMPT + STORYBOARD_STRUCTURE_PROMPT
    : CREATIVE_SCRIPT_BASE_PROMPT;
  
  console.log('[generateScriptFromIdea] 使用 prompt 类型:', originalShotCount > 0 ? '分镜Cấu trúcchế độ' : '普通创意chế độ');
