...
const PARSE_SYSTEM_PROMPT = `你是一chuyên nghiệp的剧本分析师。分析người dùng提供的剧本/故事文本，提取Cấu trúc化thông tin。

请严格按照以下JSONđịnh dạng返回kết quả（不要chứa任何其他văn bản）：
{
  "title": "故事标题",
  ...
  "scenes": [
    {
      ...
      "atmosphere": "详细Bầu không khíMô tả（如：căng thẳng压抑、ấm cúng宁静、bí ẩn阴森、悲壮肇杀）",
      "visualPrompt": "场景的详细Mô tả thị giác，用于Tạo场景ảnh khái niệm（包括光线、天气、Phong cách kiến trúc、特殊元素等，用英文）",
...
重要Yêu cầu:
1. 【角色thông tin必须详细】：不要简化角色thông tin！保留原文đang xử lý...t cả细节：
   - role: đầy đủ的Danh tính背景（如"Bắc疆侠义之士，惊鸿剑持有者，曾镇守雁城..."）
...
3. 识别多 tậpCấu trúc。如果剧本chứa"第X tập"、"Episode X"、"第X章"等标记，要拆分成多 episode
4. 如果没有明确的 tập标记，tạo单 episode chứaTất cả场景
5. 角色IDSử dụng char_1, char_2 định dạng
6. 场景IDSử dụng scene_1, scene_2 định dạng
7.  tậpIDSử dụng ep_1, ep_2 định dạng`;
...
请严格按照以下JSON数组định dạng返回kết quả（不要chứa任何其他văn bản）：
[
  {
    "sceneId": "scene_1",
    "shotSize": "Kích thước cảnh（WS/MS/CU/ECU）",
    "duration": 4.0,
    "visualDescription": "详细的đang xử lý...Mô tả，包括场景、光线、hành động nhân vật、Biểu cảm等",
    "actionSummary": "简短的动作概述",
    "cameraMovement": "镜头运动",
    "dialogue": "Thoại内容（chứa说话者和语气）",
...
5. 【音频Thiết kế】每镜头都要考虑：
   ...
   - dialogue: Thoại要chứa说话人和语气（如"天师（低沉肇立）：天地大无边..."）
6. 【thời lượng】duration 估算每镜头秒数（2-8秒，根据内容复杂度）
7. 【visualPrompt】英文Mô tả，40词内，用于图片Tạo，định dạng：
...
interface ParseOptions {
  ...
  model: string;
  language?: string;
  sceneCount?: number; // 限制场景số lượng（用于预告片等）
...
interface ShotGenerationOptions extends ParseOptions {
  ...
  shotCount?: number; // 限制总分镜số lượng（用于预告片等）
...
export async function callChatAPI(
  ...
  console.log('[callChatAPI] apiKey độ dài:', apiKey?.length || 0);
  console.log('[callChatAPI] apiKey 是否为空:', !apiKey);
  console.log('[callChatAPI] baseUrl:', baseUrl);
  console.log('[callChatAPI] systemPrompt độ dài:', systemPrompt.length);
  console.log('[callChatAPI] userPrompt độ dài:', userPrompt.length);
  ...
  if (inputTokens > modelLimits.contextWindow * 0.9) {
    const err = new Error(
      ...
      `(${modelLimits.contextWindow}) 的 90%，请缩减输入hoặcSử dụng更大上下文的模型`
    );
  ...
  return await retryOperation(async () => {
    ...
    if (!content) {
      ...
      console.error('[callChatAPI] ⚠️ API 返回空内容！诊断thông tin:');
      ...
      console.error('[callChatAPI]   reasoning_content độ dài:', reasoningContent?.length || 0);
      ...
      if (finishReason === 'sensitive' || finishReason === 'content_filter') {
        if (keyManager.handleError(403)) {
          console.warn(`[callChatAPI] 内容被an toànlọc(${finishReason})，xoay vòng key Thử lại`);
        }
        throw new Error(`内容被an toànlọc(finish_reason: ${finishReason})`);
      }
...
export async function parseScript(
  ...
  const sceneCountHint = options.sceneCount 
    ? `\n\n【重要】请仅提取最重要的 ${options.sceneCount} 场景，chọn剧情đang xử lý...表性和Thị giác冲击力的场景。`
    : '';
...
Ngôn ngữ：${options.language || 'zh'}${sceneCountHint}`;
  ...
  try {
    ...
    const scenes = (parsed.scenes || []).map((s: any, i: number) => ({
      ...
      visualPrompt: s.visualPrompt || '', // 用于场景ảnh khái niệmTạo
      tags: s.tags || [],        // 场景标签
      notes: s.notes || '',      // 场景备注
...
export async function generateShotList(
  ...
  onShotsGenerated?: (newShots: Shot[], sceneIndex: number) => void // callback streaming，每场景hoàn thành后立即通知
): Promise<Shot[]> {
...
${(scene as any).visualPrompt ? `场景Thị giácTham chiếu: ${(scene as any).visualPrompt}` : ''}
...
Thị giác风格: ${options.styleId}

=== 角色thông tin ===
${scriptData.characters.map(c => `- ${c.name}: ${c.personality || ''} ${c.appearance || ''}`).join('\n')}

=== 分镜要求 ===
1. 为该场景Tạo${shotsPerScene ? `恰好 ${shotsPerScene} ` : shotsPerSceneHint}镜头，chọn最具Thị giác冲击力的画面
2. 每镜头必须chứa：
   - shotSize: Kích thước cảnh（WS/MS/CU/ECU）
   ...
   - dialogue: Thoại（chứa说话人和语气）
   - characters: 出场角色名列表
   - keyframes: chứastartquan trọng帧的visualPrompt（英文，40词内）
3. visualDescription 要详细，包括光影、角色状态、气氛、镜头运动
4. 音频Thiết kế要具体，能复现场景Bầu không khí`;
  ...
  // 如果设置了分镜số lượng限制，截取到指定số lượng
...
const CREATIVE_SCRIPT_BASE_PROMPT = `你是一位chuyên nghiệp的影视编剧和分镜师。根据người dùng的创意输入，Tạođầy đủ的剧本。
...
重要Yêu cầu:
1. 必须chứa《标题》、**đại cương：**、**nhân vật小传：**、**第X tập**
2. 场景头định dạng：**编号 日/夜 内/外 地点**
3. 每场景必须有"nhân vật："行
4. 动作描写用 △ 开头
5. Thoạiđịnh dạng：角色tên:（动作）台词
6. MV/广告也要拆分成场景和分镜，只是内容侧重画面和Hiệu ứng âm thanh
7. Ngôn ngữ与người dùng输入giữgiống（đang xử lý...用đang xử lý...）
8. **thời đạigiống性**：đại cươngđang xử lý...确thời đại背景；nhân vật小传đang xử lý...、Kiểu tóc、đạo cụ必须严格符合该thời đại（如古代剧不得出现现代trang phục/电子产品；现代剧不得出现古代服饰）
9. **Bối cảnh thế giớigiống性**：场景地点、Phong cách kiến trúc、xã hội规则必须符合剧本设定的Bối cảnh thế giới，不得出现mâu thuẫn元素`;
...
export async function generateScriptFromIdea(
  ...
  if (durationSeconds > 0 && !sceneCount && !shotCount) {
    ...
    durationGuidance = `\n- thời lượngTham chiếu：${durationSeconds}秒视频通常chứa ${minShots}-${maxShots} 分镜，请根据内容需要自行把握Nhịp điệu`;
  }

  // 检测输入类型
  const inputType = detectInputType(idea);
  
  // 统计gốc输入đang xử lý.../场景số lượng
  // 支持多种định dạng：【镜头1】、**【镜头1：...】**、镜头1、场景1 等
...
${originalShotCount > 0 ? `- 场景số lượng：必须有 ${originalShotCount} （与gốc镜头一一对应）` : sceneCount ? `- 场景số lượng：约 ${sceneCount} ` : '- 场景số lượng：根据内容和thời lượng自行决定'}
${originalShotCount > 0 ? '' : shotCount ? `- 分镜số lượng：约 ${shotCount} ` : '- 分镜số lượng：根据内容和thời lượng自行决定'}
${styleId ? `- Thị giác风格：${styleId}` : ''}
...
4. đầy đủ的场景和Thoại${preserveStructureNote}`;
  ...
  console.log('[generateScriptFromIdea] Phát hiệngốc镜头数:', originalShotCount);
  ...
  console.log('[generateScriptFromIdea] Tạo剧本độ dài:', response.length);
...
function detectInputType(input: string): string {
  ...
  // 检测hiện có分镜Cấu trúc：【镜头X】hoặc **【镜头X】**
