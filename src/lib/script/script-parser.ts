...
请严格按照以下JSON格式返回kết quả（不要包含任何其他văn bản）：
{
  "title": "故事标题",
...
请严格按照以下JSON数组格式返回kết quả（不要包含任何其他văn bản）：
[
  {
    "sceneId": "scene_1",
...
输出格式必须严格遵循（这是Nhập系统的Tiêu chuẩn格式）：
...
7. Ngôn ngữ与người dùng输入保持giống（đang xử lý...用đang xử lý...）
...
请Tạo符合Tiêu chuẩn格式的đầy đủ剧本，包含：
1. 剧本标题
