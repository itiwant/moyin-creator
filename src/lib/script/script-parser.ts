  ...
  "scenes": [
    {
      ...
      "visualPrompt": "场景的详细视觉描述，用于Tạo场景概念图（包括光线、天气、Phong cách kiến trúc、特殊元素等，用英文）",
...
const SHOT_GENERATION_SYSTEM_PROMPT = `你是一专业的分镜师/摄影指导。为单场景Tạo电影级别的详细镜头列表（Camera Blocking）。
    ...
    "keyframes": [
      {
        "id": "kf-1-start",
        "type": "start",
        "visualPrompt": "详细的英文视觉描述（用于图片Tạo）"
      }
    ]
  }
]
...
2. 【景别缩写】WS=Viễn cảnh, MS=đang xử lý...CU=Cận cảnh, ECU=Cực cận cảnh, FS=全景
3. 【镜头运动】使用专业术语：
...
7. 【visualPrompt】英文描述，40词内，用于图片Tạo，格式：
...
const CREATIVE_SCRIPT_BASE_PROMPT = `你是一位专业的影视编剧和分镜师。根据用户的创意输入，Tạođầy đủ的剧本。
...
export async function generateScriptFromIdea(
  ...
  // 根据时长TạoTham chiếu范围（不是硬限制，是给 AI 的Tham chiếu）
